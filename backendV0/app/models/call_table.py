from sqlalchemy import Column, String, Integer, Date, Text, DateTime,Boolean,TIMESTAMP, UniqueConstraint
from sqlalchemy.dialects.postgresql import VARCHAR, TEXT, BOOLEAN, TIMESTAMP as PGTIMESTAMP, ARRAY
...
class Call(Base):
    ...
    # call_id = Column(String(255), primary_key=True, nullable=False)  # This should be the only primary key
    call_id = Column(String(255), nullable=True, unique=True)
    ...
    contact_id = Column(Integer, nullable=False)  # Remove primary_key=True and autoincrement=True
...
class Campaign(Base):
    ...
    campaign_status = Column(String(50), default='Active', nullable=False)
    contact_ids = Column(ARRAY(Integer), nullable=True)
...
class Contact(Base):
    ...
    __table_args__ = (
        UniqueConstraint('phone_number', 'user_id', name='_phone_number_user_id_uc'),
    )
